...
N_train = mnist.train.labels.shape[0]
N_valid = mnist.validation.labels.shape[0]
N_test = mnist.test.labels.shape[0]
...
# always x, y pairs
x_j = tf.placeholder(tf.float32, [None, 784], name="x_j")
y_j = tf.placeholder(tf.float32, [None, 10], name="y_j")

# Marginal distribution inputs                                                                                            
x_m = tf.placeholder(tf.float32, [None, 784], name="x_m")
y_m = tf.placeholder(tf.float32, [None, 10], name="y_m")
...
with tf.Session() as sess:
    ...
    for k in trange(len(train_subset)):
        ...
        for j in trange(trials):
            ...
            images = mnist.train.images[subset_indices,:]
            labels = mnist.train.labels[subset_indices,:]
            ...
            for i in range(args.train_iters):
                ...
                _xj, _yj = sample_pxy(images, labels, N_train_subset[k], mb)  # randomly draw <x, y> pairs
